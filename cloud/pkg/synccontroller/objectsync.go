...
func (sctl *SyncController) manageObject(sync *v1alpha1.ObjectSync) {
	...
	if sync.Spec.ObjectKind != "" && unicode.IsLower(rune(sync.Spec.ObjectKind[0])) {
		...
		oldObjectKind := sync.Spec.ObjectKind
		sync.Spec.ObjectKind = util.UnsafeResourceToKind(oldObjectKind)
		...
		klog.Warningf("Updated objectsync %s/%s ObjectKind from %s to %s.", sync.Namespace, sync.Name, oldObjectKind, sync.Spec.ObjectKind)
		if err != nil {
