...
func GetCACert(url string) ([]byte, error) {
	...
	caCert, err := ioutil.ReadAll(io.LimitReader(res.Body, constants.MaxRespBodyLength))
	if err != nil {
		return nil, err
	}
...
func (cm *CertManager) GetEdgeCert(url string, capem []byte, cert tls.Certificate, token string) (*ecdsa.PrivateKey, []byte, error) {
	...
	content, err := ioutil.ReadAll(io.LimitReader(res.Body, constants.MaxRespBodyLength))
	if err != nil {
		return nil, nil, err
	}
